...
This can be useful when you only want to deal with one package at a single time, to list all of the
devDependencies in one package for example:
...
Absolute paths, relative paths, and glob patterns are all supported – and any combination of them
