...
Add the <Link to={href.engines}>`engines`</Link> property of package.json files to also be inspected by syncpack.
...
If you don't want the Node.js version to be identical in every package but do want them all to be compatible with each other, you can use a <Link to='/config/version-groups/same-range'>Same Range</Link> Version Group. This defines an exception which only applies to Node.js, leaving anything else found under `engines` unaffected.
