...
Relax syncpack to ensure that all versions have semver ranges which all satisfy each other, instead
of the default behaviour of them having to be identical.
...
Currently the only custom policy in syncpack is "sameRange", which is what causes this version group
to be used.
