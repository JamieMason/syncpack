...
A specific occurrence of a dependency in your monorepo. For example, if three packages in your monorepo depend on `react`, there are three instances of the `react` dependency. Each instance has its own version specifier, which ideally should be consistent across all instances.
