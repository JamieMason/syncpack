...
import SupportedRanges from "@partials/supported-ranges.mdx";
import VersionGroupList from "@partials/version-group-list.mdx";
import Details from "@site/components/details.astro";

Consistent dependency versions in large JavaScript Monorepos. `syncpack` is a command-line tool to manage multiple package.json files.
...
Run the command below from the root directory of your monorepo to list every dependency installed under a [`dependencies`](HREF_DEPENDENCIES) property of a package.json file in your project. Syncpack uses your npm/pnpm/yarn/lerna workspace configuration to know which package.json files to manage.
...
When setting up a project for real, install syncpack as a dev dependency so that everyone working on your project uses the same version.

```bash
npm install syncpack --save-dev
```

Anyone on the team can now use [npm exec](HREF_NPM_EXEC) to access their local installation of syncpack:
...
For the rest of this guide I will run syncpack as if it is installed globally.
...
I'm hoping by this point that you're starting to develop an intuition for how syncpack works and how its options can be combined. Take a look at the `--help` documentation of each command to see which other options are available.
...
1. Definitely read the [`Peer Dependencies`](GUIDE_PEER_DEPENDENCIES) guide as they are very important to understand.
1. Check out [`Semver Groups`](GUIDE_SEMVER_GROUPS) to have things like version numbers in `devDependencies` always using `^` while `dependencies` always uses `~`.
1. Learn how [`Version Groups`](GUIDE_VERSION_GROUPS) can slice up your packages and dependencies in different ways. Each group can have different versioning policies to the rest of the repo, but are each kept internally consistent and valid according to their own rules.
