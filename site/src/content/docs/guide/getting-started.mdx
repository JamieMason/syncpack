...
The quickest way to run syncpack to try it is via [npx](HREF_NPX).
...
1. ### `workspace:*`
   ...
   Projects using the [pnpm workspace protocol](HREF_WORKSPACE_PROTOCOL) may use `workspace:*` when referencing a local package in `devDependencies`, but the latest semver version in `dependencies` so it is understood by other package managers.
...
```diff lang="json" title=".syncpackrc"
{
  "dependencyTypes": ["prod"],
+  "semverGroups": [
+    {
...
- Browse the [Examples](/syncpack/examples/ban-dependency-from-monorepo/) to see some common use cases and how to handle them.
- See what other functionality is provided by Version Groups <VersionGroupList/>.
- Read the guide on [Local Package Versions](/syncpack/guide/local-package-versions), which is an advanced topic.
