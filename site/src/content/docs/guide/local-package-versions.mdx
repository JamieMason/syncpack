...
import PinWorkspaceExample from '@partials/examples/pin-workspace.mdx';
import SameRangeExample from '@partials/examples/same-range.mdx';
...
1. Use the [pnpm `workspace:*`](HREF_WORKSPACE_PROTOCOL) protocol to ensure that the local workspace package of the Client is resolved when working locally on the Plugin.
...
The "local" [`dependencyType`](/syncpack/config/dependency-types) relates to the [`version`](HREF_VERSION) properties of the package.json files from your own packages being developed in your monorepo.
