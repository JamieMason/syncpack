...
description: Reference for all version specifier formats like exact, range, tag, and workspace protocols
---
...
A dependency that uses the `catalog:` protocol to reference a version defined in a shared catalog.
...
A dependency that uses the `link:` protocol to reference a local path without installing it as a symlink via the package manager.

Example: `link:../other-package`
