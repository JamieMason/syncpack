...
it('returns new Ok of default patterns when nothing is available', () => {
  ...
  expect(getPatterns(disk)(unwrap(config))).toEqual(new Ok(DEFAULT_SOURCES));
});

it('CLI --source options take precedence', () => {
  const disk = mockDisk();
  const config = getConfig(disk, { source: ['foo/package.json'] });
  expect(getPatterns(disk)(unwrap(config))).toEqual(
    new Ok(['package.json', 'foo/package.json']),
  );
});
...
describe('Yarn takes precedence after CLI --source options', () => {
  it('returns new Ok of strings when valid', () => {
    ...
    expect(getPatterns(disk)(unwrap(config))).toEqual(
      new Ok(['package.json', 'yarn/*/package.json']),
    );
  });
...
describe('Pnpm takes precedence after Yarn', () => {
  it('returns new Ok of strings when valid', () => {
    ...
    expect(getPatterns(disk)(unwrap(config))).toEqual(
      new Ok(['package.json', 'pnpm/*/package.json']),
    );
  });
...
describe('Lerna takes precedence after Pnpm', () => {
  it('returns new Ok of strings when valid', () => {
    ...
    expect(getPatterns(disk)(unwrap(config))).toEqual(
      new Ok(['package.json', 'lerna/*/package.json']),
    );
  });
