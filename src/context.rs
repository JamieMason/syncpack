...
impl Context {
  ...
  pub async fn fetch_all_updates(&mut self) {
    ...
    for update_url in self.get_unique_internal_names_of_updateable_dependencies() {
      ...
      handles.push((
        ...
        spawn(async move {
          ...
          let package_meta = client.fetch(&update_url.url).await;
  ...
  fn get_unique_internal_names_of_updateable_dependencies(&self) -> HashSet<UpdateUrl> {
    self
      .version_groups
      .iter()
