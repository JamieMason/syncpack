...
pub fn visit_formatting(ctx: Context) -> Context {
  let add_mismatch = |package: &PackageJson, mismatch: FormatMismatch| {
    let mismatch = Rc::new(mismatch);
    package.formatting_mismatches.borrow_mut().push(Rc::clone(&mismatch));
  };

  ctx.packages.all.iter().for_each(|package| {
    if ctx.config.rcfile.sort_packages || !ctx.config.rcfile.sort_first.is_empty() {
      if let Some(expected) = format::get_sorted_first(&ctx.config.rcfile, package) {
        add_mismatch(
          package,
          FormatMismatch {
            expected,
    ...
    if !ctx.config.rcfile.sort_exports.is_empty() {
      if let Some(expected) = format::get_sorted_exports(&ctx.config.rcfile, package) {
        add_mismatch(
          package,
          FormatMismatch {
            expected,
