...
export function getPatterns(
  ...
  return pipe(
    getCliPatterns(),
    Effect.flatMap(opt =>
      O.isSome(opt) ? Effect.succeed(opt) : getWorkspacePatterns(),
    ),
  ...
  function getWorkspacePatterns(): Effect.Effect<O.Option<string[]>> {
    return pipe(
      getYarnPatterns(io),
      Effect.flatMap(opt =>
        O.isSome(opt) ? Effect.succeed(opt) : getPnpmPatterns(io),
      ),
      Effect.flatMap(opt =>
        O.isSome(opt) ? Effect.succeed(opt) : getLernaPatterns(io),
      ),
