...
export class SameRangeVersionGroup extends Data.TaggedClass('SameRange')<{
  ...
  inspectAll(): Effect.Effect<Report.Version.Group[]> {
    return Effect.all(
      Object.entries(groupBy('name', this.instances)).flatMap(
        ([name, instances]) =>
          pipe(
            instances,
            Effect.partition(instance =>
              pipe(
                Effect.succeed(
                  Specifier.create(instance, instance.rawSpecifier.raw),
                ),
            ...
            Effect.map(([allMismatches, allMatches]) =>
              allMismatches.length === 0
                ? allMatches.map(thisMatch => {
                    if (
                      thisMatch.specifier.instance.strategy.name === 'local'
                    ) {
                    ...
                    const mismatches = getRangeMismatches(
                      this.ctx,
                      thisMatch,
                      allMatches,
                    );
...
function matchesRange(ctx: Ctx, a: Report.Valid, b: Report.Valid): boolean {
  const loose = true;
  return intersects(
    unwrapSemver(ctx, a.specifier),
    unwrapSemver(ctx, b.specifier),
    loose,
  );
}
...
function unwrapSemver(ctx: Ctx, specifier: Specifier.Any): string {
  ...
  if (specifier._tag === 'WorkspaceProtocol') {
    return Effect.runSync(
      (specifier as WorkspaceProtocolSpecifier).getSemverEquivalent(ctx),
    );
  }
