use {
  crate::{
    ...
    minor::Minor, range::Range, range_major::RangeMajor, range_minor::RangeMinor, semver_range::SemverRange, tag::Tag,
...
impl Specifier {
  ...
  pub fn get_semver_number(&self) -> Option<&str> {
    match self {
      ...
      Self::WorkspaceProtocol(s) => s.as_resolved().and_then(|spec: &Rc<Specifier>| spec.get_semver_number()),
    ...
    match self {
      ...
      Self::WorkspaceProtocol(s) => {
        ...
        s.as_resolved().and_then(|spec: &Rc<Specifier>| spec.get_node_range()).or_else(|| {
          use crate::{semver_range::SemverRange, workspace_specifier::WorkspaceSpecifier};
  ...
  pub fn with_node_version(&self, node_version: &node_semver::Version) -> Option<Rc<Self>> {
    ...
    match self {
      ...
      Self::WorkspaceProtocol(wp) => {
        ...
        wp.as_resolved().and_then(|resolved: &Rc<Specifier>| {
